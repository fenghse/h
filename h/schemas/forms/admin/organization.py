...
def validate_logo(node, value):
    if not (len(value) <= ORGANIZATION_LOGO_MAX_CHARS):
        raise colander.Invalid(node, _(
            "Logo is larger than {} characters".format(ORGANIZATION_LOGO_MAX_CHARS)))
    try:
    ...
    if _strip_xmlns(root.tag) != 'svg':
        raise colander.Invalid(node, _('Logo is does not start with <svg> tag'))
