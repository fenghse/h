...
from h.schemas.forms.admin.organization import OrganizationSchema, ORGANIZATION_LOGO_MAX_CHARS
...
class TestOrganizationSchema(object):
    ...
    def test_it_raises_if_logo_is_too_long(self, org_data, bound_schema):
        org_data['logo'] = '<svg xmlns="http://svg.com">{}</svg>'.format('a' * ORGANIZATION_LOGO_MAX_CHARS + 'b')

        with pytest.raises(
            colander.Invalid,
            match="larger than {} characters".format(ORGANIZATION_LOGO_MAX_CHARS)
        ):
            bound_schema.deserialize(org_data)
    ...
    def test_it_raises_if_logo_is_not_svg(self, org_data, bound_schema):
        ...
        with pytest.raises(
            colander.Invalid,
            match="does not start with <svg> tag",
        ):
            bound_schema.deserialize(org_data)
